      ...
      <div className="mb-8 space-y-4 animate-fade-in" style={{ animationDelay: "100ms" }}>
        <div className="bg-bg-card border border-border rounded-xl px-5 py-4">
          <p className="text-[15px] leading-relaxed">
            If you&apos;re feeling too awkward to even <span className="font-semibold text-white">snap the pic</span>, how are you going to walk up and talk to them? Taking the photo is your first step — it breaks through that initial wall of fear.
          </p>
        </div>

        <div className="bg-bg-card border border-border rounded-xl px-5 py-4">
          <p className="text-[15px] leading-relaxed">
            Think about how incredible it would be to actually <span className="font-semibold text-white">approach and connect</span> with that person. That one memorable experience is worth infinitely more than $15.
          </p>
        </div>

        <div className="bg-bg-card border border-border rounded-xl px-5 py-4">
          <p className="text-[15px] leading-relaxed">
            Cold approaching your crush and overcoming your nerves to talk to them is one of the <span className="font-semibold text-white">most fulfilling experiences</span> of your life — something you&apos;ll remember forever. Don&apos;t let hesitation hold you back.
          </p>
        </div>
      </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              <div className="flex items-center gap-2">
                ...
                {!(isActive && isYearly) && (
                  <span className={`text-[11px] font-semibold px-2 py-0.5 rounded-full ${
                    isActive ? "bg-bg-input text-text-muted" : "bg-white/10 text-white"
                  }`}>
                    SAVE 33%
                  </span>
                )}
                {isActive && isYearly && (
                  <span className="text-[11px] font-semibold bg-black/10 text-black px-2 py-0.5 rounded-full">
                    CURRENT
                  </span>
                )}
              </div>
              <div className="flex items-baseline gap-1.5 mt-1.5">
                ...
                <span className={`text-[14px] ${isActive && isYearly ? "text-black/50" : "text-text-muted"}`}>/month</span>
              </div>
              <p className={`text-[13px] mt-1 ${isActive && isYearly ? "text-black/40" : "text-text-muted"}`}>
                $120 billed annually
              </p>
            </div>
            {isActive && isYearly && (
              <Check size={22} strokeWidth={2.5} className="text-black" />
            )}
          </div>
          {!isActive && (
            <button
              ...
              disabled={!!loading}
              className="mt-4 w-full bg-white text-black py-3 rounded-lg font-medium text-[14px] press disabled:opacity-60"
            >
